...
import hoang.dqm.codebase.databinding.DifLayoutBinding
...
abstract class BaseRecyclerViewItemAdapter<T, VB : ViewBinding> :
    ...
    var onPlaceholderClickListener: (() -> Unit)? = null
    ...
    fun OnPlaceholderClickListener(listener: () -> Unit) {
        onPlaceholderClickListener = listener
    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<VB> {
        return when (viewType) {
            ...
            RecyclerViewType.TYPE_PLACEHOLDER.value -> (BaseViewHolder(
                DifLayoutBinding.inflate(
    ...
    open fun bindViewClickListener(viewHolder: BaseViewHolder<VB>, _viewType: Int) {
        viewHolder.itemView.singleClick {
            if (!isCheckClickItem()) return@singleClick
            ...
            when (val item = getItem(position)) {
                ...
                ItemList.Placeholder -> {
                    onPlaceholderClickListener?.invoke()
                }
            }
        }
        ...
        try {
            when (dataList[position]) {
                is ItemList.DataItem<T> -> {
                    bindData(
                        holder.binding,
                        (dataList[position] as ItemList.DataItem<T>).item,
                        position
                    )
                }
